...
public class Session {
	...
	public int vport = 5006;
	public int aport = 5004;
	...
	public void addVideoTrack(int encoder, int camera, VideoQuality videoQuality, boolean flash) throws IllegalStateException, IOException {
		...
		case VIDEO_H264:
			...
			//VideoStream.mActivity=mActivity;
			break;
		case VIDEO_H263:
		...
		if (stream != null) {
			...
			stream.setDestination(destination, vport);
	...
	public void addAudioTrack(int encoder) {
		...
		if (stream != null) {
			stream.setDestination(destination, aport);
		...
		synchronized (LOCK) {
			for (int i=0;i<tracks.size();i++) {
				...
				if ((track.type == Track.VIDEO && !cameraInUse) || (track.type == Track.AUDIO && !micInUse)) {
					...
					sessionDescriptor += "a=control:trackID="+(i+1)+"\r\n";
				}
			}
		}
