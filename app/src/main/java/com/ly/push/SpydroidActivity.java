...
public class SpydroidActivity extends Activity implements OnSharedPreferenceChangeListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Session.user = settings.getString("user", "");
        Session.pass = settings.getString("pass", "");
        ...
        wl = pm.newWakeLock(PowerManager.SCREEN_DIM_WAKE_LOCK, "com.ly.push.spydroid.wakelock");
        ...
        findViewById(R.id.remotebtn).setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View v) {
				new Thread(){
					public void run() {
						new RemoteServer(((EditText)findViewById(R.id.remote)).getText().toString(),554,handler);
					};
				}.start();
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
    	...
    	{
    		Session.setUsername(sharedPreferences.getString("user", ""));
    	}
    	else if(key.equals("pass"))
    	{
    		Session.setPassword(sharedPreferences.getString("pas", ""));
    	}
    }
